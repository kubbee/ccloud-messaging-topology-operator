						...
						if environmentId, z := connCreds.Data("environmentId"); z {
							...
							if environment, w := connCreds.Data("environment"); w {
								...
								if topic, err := services.BuildTopic(topic, string(environmentId), string(clusterId), &logger); err != nil {
									...
									if connCredsKafka := r.readCredentials(ctx, kafkaTopic.Spec.KafkaClusterResource.Namespace, "kafka-"+string(environment), 2); connCredsKafka != nil {
										if connCredsSR := r.readCredentials(ctx, kafkaTopic.Spec.KafkaClusterResource.Namespace, "schemaregistry-"+string(environment), 3); connCredsSR != nil {
